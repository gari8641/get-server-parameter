...
echo ufwを使用する。 >> $filename
echo  >> $filename
echo \#\#	ufwの設定ルール >> $filename
echo - 必要なポート又はサービスのみ開放する。 >> $filename
echo  >> $filename
echo \#\#	ufwの設定内容 >> $filename
echo  >> $filename
echo '```' >> $filename
echo \# ufw status >> $filename
ufw status >> $filename
echo '```' >> $filename
...
echo '```' >> $filename
echo \# apt list --installed >> $filename
...
echo  >> $filename
echo /etc/rsyslog.confはデフォルトのまま。>> $filename
echo  >> $filename
...
echo '```' >> $filename
echo  >> $filename
echo /etc/rsyslog.d/ 配下は24-07-30_to-rsyslog-server.confファイルを新規追加。>> $filename
echo  >> $filename
echo /etc/rsyslog.d/24-07-30_to-rsyslog-server.confの内容：>> $filename
echo '```' >> $filename
echo \# cat /etc/rsyslog.d/24-07-30_to-rsyslog-server.conf >> $filename
cat /etc/rsyslog.d/24-07-30_to-rsyslog-server.conf >> $filename
echo '```' >> $filename
